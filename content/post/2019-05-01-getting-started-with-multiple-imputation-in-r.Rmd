...
Always check how your variables are imputed with the `head` and `tail` option. If you still see missing values after imputation, this means the algorithm didn't work as intended. There shouldn't be huge differences between your analysis pre-imputation and after-imputation, unless missing values are highly affecting your analysis (in that case, it might be useful to think about other strategies to collect more data). I'd suggest you impute the whole dataset, rather than only the variable of interest. 
